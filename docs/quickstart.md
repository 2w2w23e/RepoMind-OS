...
- 询问用户希望先使用 minimal setup（最小设置）还是 custom roles（自定义角色）；
- 询问是否有已有角色、prompt、上下文或偏好需要导入和评估；
...
当 Project Governor 需要另一个角色协作时，它会使用以下模板准备 Role Task Packet（角色任务包）：
...
对于文件修改任务，Project Governor 或 Prompt Architect 应该准备包含以下内容的 Codex prompt：
...
第一个 GPT 窗口不能在总结 BOOT.md 后停止。它必须主动进入 bootstrap mode（启动模式）：

- 判断当前是 first-window 还是 existing-session；
- 询问用户是否已有角色、上下文或偏好；
- 如果是第一窗口，继续 FIRST_WINDOW_PROTOCOL；
- 输出下一步可执行的 bootstrap 行动，而不是被动等待。
